...
This online-only example compares the first-order Extended Kalman Filter
(EKF1), the second-order Extended Kalman Filter (EKF2), and the Unscented
Kalman Filter (UKF) in a setting where the role of curvature is transparent.
Consider the scalar state-space model
\[
x_t = 0.08 + 0.85x_{t-1} + \varepsilon_t,
\qquad
y_t = x_t + 0.45x_t^2 + \eta_t,
\]
...
```{r fig_EKF_comparison, message=FALSE, warning=FALSE, fig.width=8, fig.height=8}
...
correction.ekf2 <- curvature * fit.ekf2$predicted.cov[1, 1, ]
correction.ukf <- fit.ukf$predicted.measurement[, 1] -
  measurement(fit.ukf$predicted.mean[, 1], seq_len(TT))

plot(correction.ekf2, type = "l", lwd = 2,
     xlab = "Date", ylab = "Correction",
     main = "(b) Curvature contribution to the predictive measurement mean",
     las = 1)
grid()
abline(h = 0, lty = 3, col = "darkgrey")
points(ukf.dates, correction.ukf[ukf.dates],
       pch = 21, bg = "white", cex = 0.75)
legend(
  "topright", legend = c("EKF2 correction", "UKF correction"),
  lty = c(1, NA), pch = c(NA, 21), pt.bg = "white",
  lwd = c(2, NA), bty = "n"
)
```

The second panel displays the curvature term that EKF1 omits when the filters
are evaluated at the same Gaussian predictive approximation. The UKF markers
lie on the EKF2 line. The following error measures confirm that, in this
experiment, incorporating curvature improves latent-state recovery and that
EKF2 and the UKF produce the same recursion up to machine precision.

```{r table_EKF_comparison}
...
knitr::kable(
  comparison, digits = 3,
  caption = "Accuracy of EKF1, EKF2, and UKF in the quadratic-measurement example."
)
```
