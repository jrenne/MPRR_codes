...
# Report the cumulative variance explained by the first principal components.
pca_variance_share <- cumsum(PCA$sdev^2/sum(PCA$sdev^2))
knitr::kable(
  data.frame(
    principal_component = 1:3,
    cumulative_variance_share = round(pca_variance_share[1:3], 4)
  ),
  col.names = c("Principal component", "Cumulative variance share")
)
...
xsr_variance_share <- cumsum(PCA_xsr$sdev^2/sum(PCA_xsr$sdev^2))
knitr::kable(
  data.frame(
    principal_component = 1:3,
    cumulative_variance_share = round(xsr_variance_share[1:3], 4)
  ),
  col.names = c("Principal component", "Cumulative variance share")
)
...
```{r fig_YC_example_TSTP, fig.width = 8, fig.height = 3.5}
...
```{r simple_TIPS_decomposition_check}
...
# Retain numerical identity checks without printing them in the rendered chapter.
nominal_eh_identity_error <- max(abs(nom_EH_check - nom_YC_EH_sim))
real_short_identity_range <- range(
  real_short_sim - (real_short_delta0 + state_sim %*% real_short_delta1)
)
```
