...
This example compares standard structural and reduced-form views of credit spreads.
The empirical series combine two ingredients: a credit-spread component and a default-rate component. The credit spread is computed from the Moody's Seasoned Aaa and Baa corporate bond yields available on FRED (`DAAA` and `DBAA`), using annual averages and the difference `DBAA - DAAA`. The default component contains global corporate default counts and default rates from S&P Global Ratings' "Global corporate default summary" table, sourced by S&P Global Ratings to S&P Global Ratings Credit Research & Insights and S&P Global Market Intelligence's CreditPro. The 2024 update corresponds to S&P Global Ratings, "Default, Transition, and Recovery: 2024 Annual Global Corporate Default And Rating Transition Study" (March 27, 2025); the same global summary table is also reproduced in related regional S&P default-transition studies.^[The underlying observations are not distributed with DTAM or with the public version of this Rbookdown project. Readers wishing to reproduce them should consult the source pages for [DAAA](https://fred.stlouisfed.org/series/DAAA), [DBAA](https://fred.stlouisfed.org/series/DBAA), [FRED terms](https://fred.stlouisfed.org/legal/), and the relevant S&P Global Ratings corporate default-transition study, subject to the applicable permissions.]

```{r fig_standardCredit1, fig.width=5.5, fig.height=7}
# Purpose: This example compares standard structural and reduced-form views of credit spreads
library(DTAM)

private_credit_file <- "private_data/Credit_spds.rda"
precomputed_figure <- if (knitr::is_latex_output()) {
  "_main_files/figure-latex/fig_standardCredit1-1.pdf"
} else {
  "_main_files/figure-latex/fig_standardCredit1-1.pdf"
}

if (!file.exists(private_credit_file) && file.exists(precomputed_figure)) {
  knitr::include_graphics(precomputed_figure)
} else {
  if (!file.exists(private_credit_file)) {
    stop("Private credit-spread data not found: ", private_credit_file, call. = FALSE)
  }
  load(private_credit_file)

RR <- .6
lambda       <- (0.5*Credit_spds$Investment.grade.default.rate + 0.5*Credit_spds$Default.rate)/100
spd_BBB_AAA  <- Credit_spds$BBB_AAA_spd/100
...
loss4optim <- function(param){
  ...
  x <- (spd_BBB_AAA - b_spd - a_spd[2]*lambda)/a_spd[1]
  ...
  penalty_neg_x <- 10000000*sum(abs(x[x<0]))
  ...
  penalty_mean_spd <- 1000000000*abs(avg_spd - mean(spd_BBB_AAA))
  
  # penalty for stdv of spreads:
  stdv_spd <- sqrt(t(a_spd) %*% Var_Y %*% a_spd)
  penalty_var_spd <- 10000*abs(stdv_spd - sd(spd_BBB_AAA))
  ...
  Autocor_spd <- cor(spd_BBB_AAA[2:T],spd_BBB_AAA[1:(T-1)])
...
plot(Credit_spds$year,Credit_spds$Default.rate/100,
     ylim=c(0,5),col="white",
     xlab="",ylab="in percent",las=1,
     main=expression(paste("(a) Default rate and credit spread",sep="")))
grid()
lines(Credit_spds$year,Credit_spds$Default.rate,lwd=2,lty=2)
lines(Credit_spds$year,Credit_spds$Investment.grade.default.rate,lwd=2,lty=1)
lines(Credit_spds$year,Credit_spds$BBB_AAA_spd,col="dark grey",type="b",lwd=2)

legend("topleft",
       legend=c(expression(paste("Default rate (average)",sep="")),
                expression(paste("Default rate (investment grade)",sep="")),
                expression(paste("Credit spread",sep=""))),
       lty=c(2,1,NaN),
       pch=c(NaN,NaN,1),
       col=c("black","black","dark grey"),
       lwd = 2,bty = "n",seg.len = 3)

plot(Credit_spds$year,100*lambda,
     ylim=c(0,3),col="white",
     xlab="",ylab="in percent",las=1,
     main=expression(paste("(b) Factors ",x[t]," and ",lambda[t],sep="")))
grid()
lines(Credit_spds$year,100*lambda,lwd=2,type="b",pch=3)
lines(Credit_spds$year,100*x,col="dark grey",lwd=2,type="b",pch=0)
...
grid()
lines(Credit_spds$year,100*PD_P_H1,lwd=2,lty=1)
lines(Credit_spds$year,100*PD_Q_H1,lwd=2,lty=3)
lines(Credit_spds$year,100*PD_P_H2,lwd=2,lty=1,col="dark grey")
lines(Credit_spds$year,100*PD_Q_H2,lwd=2,lty=3,col="dark grey")
