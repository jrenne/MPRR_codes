...
Credit_spds <- merge(spread_data, loss_data, by = "year", all = FALSE)
stopifnot(nrow(Credit_spds) == 39,
          !anyDuplicated(Credit_spds$year),
          all(Credit_spds$HQM_TNC_spd > 0),
          all(Credit_spds$default_intensity_pct >= 0))

RR <- .6
lambda       <- Credit_spds$default_intensity_pct/100
spd_HQM_TNC  <- Credit_spds$HQM_TNC_spd/100
stopifnot(max(abs(lambda*(1-RR) - Credit_spds$expected_loss_bp/10000)) < 1e-10)
...
plot(Credit_spds$year,Credit_spds$default_intensity_pct,
     ylim=c(0,1.2*max(c(Credit_spds$default_intensity_pct,
                        Credit_spds$HQM_TNC_spd))),col="white",
     xlab="",ylab="in percent",las=1,
     main=expression(paste("(a) Default-intensity indicator and 20-year credit spread",sep="")))
grid()
lines(Credit_spds$year,Credit_spds$default_intensity_pct,lwd=2,lty=2)
lines(Credit_spds$year,Credit_spds$HQM_TNC_spd,col="dark grey",type="b",lwd=2)
