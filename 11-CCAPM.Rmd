...
#          Average excess return, sd(excess return), sd(dc),
#          Cor(dc, excess return), Cov(dc, excess return),
...
for(period in 1:dim(matrix_periods)[1]){
  ...
  for(ctry in list.of.JST.ctries){
    ...
    # Use one common set of country-year observations for every moment entering
    # the empirical CCAPM approximation.
    DATA.moments <- DATA.reduced[
      complete.cases(DATA.reduced[,c("dc","Excess_return","rstir")]),]
    sample <- range(DATA.moments$year)

    mean_er <- mean(DATA.moments$Excess_return)
    sd_er   <- sd(DATA.moments$Excess_return)
    sd_dc   <- sd(DATA.moments$dc)
    cor_er_dc <- cor(DATA.moments$dc,DATA.moments$Excess_return)
    cov_er_dc <- cov(DATA.moments$dc,DATA.moments$Excess_return)
    SR <- mean_er/sd_er
    mean_Rf <- mean(DATA.moments$rstir)
...
legend("topleft",
       c("Short-term real rate","Term structure of real yields",
