...
The source workbook is available from [SIFMA's U.S. Fixed Income Securities Statistics page](https://www.sifma.org/research/statistics/us-fixed-income-securities-statistics). The workbook identifies the underlying sources for the series used here as the U.S. Department of the Treasury and the Bureau of the Fiscal Service. Because the workbook is subject to SIFMA's terms of use, it is not redistributed with this repository; the code below can be run after an authorized copy has been placed in the local `private_data` directory.
...
sifma_path <- "private_data/ta-us-treasury-sifma.xlsx"
if (!file.exists(sifma_path)) {
  stop(
    paste0(
      "The SIFMA source workbook is not distributed with this repository. ",
      "Obtain an authorized copy and save it as ", sifma_path, "."
    ),
    call. = FALSE
  )
}
...
ta_us_treasury_sifma <- read_excel(sifma_path, sheet = "Issuance", skip = 5)
