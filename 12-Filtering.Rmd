...
alpha <- .3*sd(Y_t)
beta <- 0.1
param <- c(a,b,c,alpha,beta,log(rho/(1-rho)))
  ...
  for(i in 1:nb_loop){
    ...
    for(method in c("Nelder-Mead","nlminb")){
      res.optim <- optimx(par=param,
                          ...
                          method=method,
                          control=list(trace=TRUE,
                                       maxit=ifelse(method=="Nelder-Mead",200,10),
                                       kkt = FALSE))
      print(paste("--- Log-lik (",method,"): ",round(res.optim$value,2)," ---",sep=""))
...
plot(vec_dates,Y_t,col="white",xlab="",ylab="",las=1,
     main=expression(paste("(b) ",tau[t]," and ",tau[t]^2,sep="")))
grid()
...
lines(vec_dates,b_x_tau,col="black",lwd=2)
lines(vec_dates,c_x_tau2,col="black",lwd=2,lty=3)
legend("topleft",
